...
- **多域支持**: 自动发现 `configs/` 下多个业务域（`metrics.yaml` → ecommerce, `bank_metrics.yaml` → bank），每个域独立 DB + Milvus + RAG + DomainContext
...
用户请求
  ...
  → LangGraph StateGraph 执行:
    ...
    → decompose（复杂查询改写，对比/同比/趋势 → 单 DSL 形式）
...
- **Trace 累加**: `Annotated[list[dict], add_to_list]` reducer 自动累积每个节点的 trace 记录
...
| `NL2DSL_MILVUS_URI` | 否 | Milvus Lite 本地文件路径，默认项目根目录 `data/milvus_lite.db` |
| `NL2DSL_DB_URL` | 否 | 数据库连接串，默认项目根目录 `data/nl2dsl.db` |
