...
.rag_sync_state.json        # 各 YAML 上次同步时间戳（首次启动自动生成）
milvus_lite.db/             # 向量库数据目录（首次启动自动生成）
```
...
用户请求
  ...
  → LangGraph StateGraph:
    → clarification         歧义检测
    → validation 子图       RAG 检索 → LLM 生成 DSL → 校验 → 失败自动修正循环
...
| 集合 | 来源 YAML | 内容 | 检索方式 |
|------|---------|------|----------|
| `schema` | metrics.yaml | 维度 + 指标定义 | jieba 切词关键词检索 |
...
**自检同步机制**：后端启动时对比每个 YAML 的 mtime 和 `.rag_sync_state.json` 中的记录，过期则增量重灌；都最新则跳过 BGE 模型加载，启动毫秒级返回。
...
| GET | `/api/v1/debug/rag?q=...` | 调试：查看 RAG 检索结果 |
