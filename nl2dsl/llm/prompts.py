...
- 指标的 `alias` 必须是【可用指标】清单里出现过的名字，这是指标的**唯一权威标识**
- 指标的物理列由系统根据 `alias` 对应的注册表达式自动解析，**不要自己猜测物理列名**：
  - 能从【可用指标】清单确定指标时，`field` 填该指标名（与 `alias` 相同）即可
  - 严禁发明清单中不存在的物理列名（如把 `pay_amount` 写成 `order_amount` 之类）
  - 仅当指标确实是清单外的原始字段聚合时，才填实际存在的物理列名
...
- 必须是数组，每个元素包含：
  ...
  - `field`: 原始字段名（不要带 SUM/AVG/COUNT 等函数前缀）。能对应到【可用指标】清单时，填该指标名；系统会按 `alias` 的注册表达式解析物理列，不要猜测或编造物理列名
...
### post_process（高级分析后处理，可选）
仅用于 SQL 基础聚合完成后的受控结果加工，禁止用自由文本表达式。

- 各组 TopN：
  `{"type":"group_top_n","metric":"sales_amount","group_by":["category"],"top_n":1,"direction":"desc"}`
  示例：“各品类销售额最高的产品”。dimensions 必须同时包含 `category` 和 `product_name`。
- 占比：
  `{"type":"proportion","metric":"sales_amount","output_field":"sales_amount_proportion"}`
  示例：“各品类销售额占总销售额的比例”。占比为 0-1 小数，分母是返回分组指标之和。
- post_process.metric 必须是 metrics 中已输出的 alias。
- group_by 必须是 dimensions 中已输出的维度。
...
DSL_JSON_SCHEMA = json.dumps(
    {
        ...
        "properties": {
            ...
            "post_process": {
                ...
                "properties": {
                    "type": {"enum": ["group_top_n", "proportion"]},
                    "metric": {"type": "string"},
